...
function userPrompt(args: {
  ...
  // Подсказка для AI по типу скрипта на основе первого пункта чек-листа
  // (title содержит «металлоконструкции» → это МК, иначе предполагаем МП)
  const isMkChecklist = checklist && checklist.some(
    (c) => c.id === "what_metal" && /металлоконструкц/i.test(c.title ?? "")
  );
  const scriptTypeHint = checklist && checklist.length > 0
    ? (isMkChecklist
        ? `\nПодсказка для определения типа скрипта: данный чек-лист относится к направлению МК (менеджер-консультант) — металлоконструкции, строительство, монтаж. Типичные темы МК: ангар, склад, каркасное здание, монтаж, сэндвич-панели, чертёж, проект, под ключ, исполнительная документация. Не путать с МП.`
        : `\nПодсказка для определения типа скрипта: данный чек-лист относится к направлению МП (менеджер продаж) — металлопрокат, поставки. Типичные темы МП: «стоимость», «цена», «купить», «заказ», «КП», «счёт», «договор», «арматура», «балка», «труба», «лист».`)
    : "";

  const checklistBlock =
    checklist && checklist.length > 0
      ? `Чек-лист QC (оцени каждый пункт от 0 до 1; 1 = выполнено полностью, 0 = не выполнено вообще, weight — важность от 1 до 5):${scriptTypeHint}
