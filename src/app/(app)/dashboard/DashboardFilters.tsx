...
export function DashboardFilters({ managers, basePath = "/dashboard" }: { managers?: ManagerOption[]; basePath?: string }) {
  ...
  // Синхронизируем локальный state from/to с URL-параметрами при их изменении
  // (в т.ч. после дефолт-редиректа на «Сегодня») — иначе подсветка активного
  // пресета не срабатывает, т.к. useState инициализируется только при mount.
  useEffect(() => {
    setFrom(fromParam);
    setTo(toParam);
  }, [fromParam, toParam]);
  ...
  return (
    ...
    >
      ...
      {/* «Только с CRM» — после «За всё время» в той же строке. Collapsible (мобайл под «Ещё»). */}
      <button
        type="button"
        onClick={toggleCrm}
        disabled={pending}
